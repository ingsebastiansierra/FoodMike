...
const ShortCard = ({
    ...
    const handleDoubleTap = () => {
        ...
        if (lastTap.current && (now - lastTap.current) < DOUBLE_TAP_DELAY) {
            ...
            // Ejecutar en el siguiente ciclo para evitar warning
            ...
            lastTap.current = now;
            // Si no hay segundo tap en 300ms, pausar/reproducir
            setTimeout(() => {
    ...
    const handleLikeAnimation = () => {
        // Usar requestAnimationFrame para evitar actualizar durante render
        requestAnimationFrame(() => {
    ...
    return (
        <View style={styles.container}>
            <Video
                ...
                onError={(error) => {
                    console.warn('Video playback error (codec may not be supported):', error);
                    setIsLoading(false);
                    setHasError(true);
                }}
            ...
            >
                ...
                {/* Animación de like al hacer doble tap */}
                {showLikeAnimation && (
                    <Animated.View
                        style={[
...
const styles = StyleSheet.create({
    ...
    restaurantInfo: {
        position: 'absolute',
        bottom: 120, // Más espacio para el bottom navigator
        left: SPACING.md,
        right: 80,
    },
    restaurantHeader: {
